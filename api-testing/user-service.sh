...
code=$(request GET "/api/v1/users/$CUSTOMER_ID" "$TMP_DIR/user_by_id.out" "${AUTH[@]}")
assert_code "$code" "200" "get user by id" "$TMP_DIR/user_by_id.out"

code=$(request GET "/api/v1/users/by-email?email=$CUSTOMER_EMAIL" "$TMP_DIR/user_by_email.out" "${AUTH[@]}")
assert_code "$code" "200" "get user by email" "$TMP_DIR/user_by_email.out"

code=$(request GET "/api/v1/users/email-exists?email=$CUSTOMER_EMAIL" "$TMP_DIR/email_exists.out" "${AUTH[@]}")
...
code=$(request GET "/api/v1/users/$CUSTOMER_ID/exists" "$TMP_DIR/user_exists.out" "${AUTH[@]}")
assert_code "$code" "200" "internal exists" "$TMP_DIR/user_exists.out"

code=$(request GET "/api/v1/users/$SELLER_ID/role" "$TMP_DIR/user_role.out" "${AUTH[@]}")
assert_code "$code" "200" "internal role" "$TMP_DIR/user_role.out"

code=$(request GET "/api/v1/users/$CUSTOMER_ID/validate" "$TMP_DIR/user_validate.out" "${AUTH[@]}")
assert_code "$code" "200" "internal validate" "$TMP_DIR/user_validate.out"

code=$(request PATCH "/api/v1/users/$CUSTOMER_ID/deactivate" "$TMP_DIR/user_deactivate.out" "${AUTH[@]}")
assert_code "$code" "204" "deactivate user" "$TMP_DIR/user_deactivate.out"

code=$(request PATCH "/api/v1/users/$CUSTOMER_ID/activate" "$TMP_DIR/user_activate.out" "${AUTH[@]}")
assert_code "$code" "204" "activate user" "$TMP_DIR/user_activate.out"

code=$(request GET "/api/v1/users/stats" "$TMP_DIR/user_stats.out" "${AUTH[@]}")
assert_code "$code" "200" "user stats" "$TMP_DIR/user_stats.out"

code=$(request DELETE "/api/v1/users/$CUSTOMER_ID" "$TMP_DIR/user_delete.out" "${AUTH[@]}")
assert_code "$code" "204" "soft delete user" "$TMP_DIR/user_delete.out"

code=$(request GET "/api/v1/users" "$TMP_DIR/users_list_2.out" "${AUTH[@]}")

...
const UserSidebar = ({ currentUser, users, orders }) => {
  ...
  return (
    <aside className="glass-panel rounded-3xl border p-5 md:p-6">
      <div className="mb-5 flex items-center gap-2 text-slate-900">
        <RoleIcon className="h-5 w-5 text-cyan-700" />
        <h2 className="text-lg font-semibold">{roleTitle[currentUser?.role] || 'Thông tin user'}</h2>
      </div>

      <div className="rounded-2xl border border-slate-200 bg-white/70 p-4">
        <p className="text-xs uppercase tracking-widest text-slate-400">Tài khoản hiện tại</p>
        <p className="mt-2 text-lg font-bold text-slate-900">{currentUser?.name}</p>
        <p className="text-sm text-slate-600">{currentUser?.email}</p>
        <p className="mt-2 inline-block rounded-full bg-cyan-100 px-2 py-1 text-xs font-semibold text-cyan-700">
          {currentUser?.role}
        </p>
      </div>
      ...
      {isAdmin && (
        <div className="mt-4 rounded-2xl border border-slate-200 bg-white/70 p-4">
          <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-slate-700">
            <Users2 className="h-4 w-4 text-cyan-700" />
